...
const CONFIG_FILE_NAME: &str = "deepl-config.json";

#[derive(Clone, Debug, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct DeepLConfigStatus {
    pub configured: bool,
...
#[derive(Clone, Debug, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct DeepLConnectionTest {
    pub ok: bool,
    pub endpoint: String,
    pub message: String,
}
...
fn config_path(app: &AppHandle) -> AppResult<PathBuf> {
    app.path()
        ...
        .map_err(|_| {
            AppError::new(
                ErrorCode::DiskFull,
                "DeepL config directory could not be resolved.",
            )
        })
}

fn read_stored_config(app: &AppHandle) -> AppResult<Option<StoredDeepLConfig>> {
    ...
    let raw = fs::read_to_string(path)
        .map_err(|_| AppError::new(ErrorCode::DecryptFailed, "DeepL config could not be read."))?;
    serde_json::from_str::<StoredDeepLConfig>(&raw)
        .map(Some)
        .map_err(|_| {
            AppError::new(
                ErrorCode::DecryptFailed,
                "DeepL config is not a valid config file.",
            )
        })
}

fn write_stored_config(app: &AppHandle, config: &StoredDeepLConfig) -> AppResult<()> {
    let path = config_path(app)?;
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).map_err(|_| {
            AppError::new(
                ErrorCode::DiskFull,
                "DeepL config directory could not be created.",
            )
        })?;
    }
    let raw = serde_json::to_string_pretty(config).map_err(|_| {
        AppError::new(
            ...
            "DeepL config could not be serialized.",
        )
    })?;
    fs::write(path, raw)
        .map_err(|_| AppError::new(ErrorCode::DiskFull, "DeepL config could not be saved."))
}

fn local_api_key(app: &AppHandle) -> AppResult<Option<(String, StoredDeepLConfig)>> {
...
pub fn deepl_api_key(app: &AppHandle) -> AppResult<String> {
    ...
    env_api_key().ok_or_else(|| {
        AppError::new(
            ...
            "DeepL API Key is not configured. Save it in Settings or set DEEPL_API_KEY.",
        )
    })
}

pub fn endpoint_base(api_key: &str) -> &'static str {
...
pub fn status(app: &AppHandle) -> AppResult<DeepLConfigStatus> {
    if let Some((api_key, stored)) = local_api_key(app)? {
        return Ok(DeepLConfigStatus {
            configured: true,
            source: "local",
    ...
    if let Some(api_key) = env_api_key() {
        return Ok(DeepLConfigStatus {
            configured: true,
            source: "environment",
            storage: "DEEPL_API_KEY",
            masked_key: Some(mask_api_key(&api_key)),
            updated_at: None,
        });
    }

    Ok(DeepLConfigStatus {
        configured: false,
        source: "none",
...
pub fn save_api_key(app: &AppHandle, api_key: String) -> AppResult<DeepLConfigStatus> {
...
pub fn clear_api_key(app: &AppHandle) -> AppResult<DeepLConfigStatus> {
    let path = config_path(app)?;
    match fs::remove_file(path) {
        ...
        Err(_) => {
            return Err(AppError::new(
                ErrorCode::DiskFull,
                "DeepL config could not be cleared.",
            ));
        }
    }
    status(app)
...
pub async fn test_api_key(
    ...
    let endpoint = endpoint_base(&api_key);
    let client = Client::builder()
        ...
        .map_err(|_| {
            AppError::new(
                ...
                "DeepL test client could not be initialized.",
            )
        })?;
    ...
    let response = client
        ...
        .map_err(|error| {
            ...
            AppError::new(code, "DeepL connection test could not be completed.")
        })?;

    let status = response.status();
    if status.is_success() {
        return Ok(DeepLConnectionTest {
            ok: true,
            endpoint: endpoint.to_owned(),
            message: "DeepL API Key is valid.".to_owned(),
        });
    }
    ...
    let (code, message) = match status {
        StatusCode::UNAUTHORIZED | StatusCode::FORBIDDEN => (
            ErrorCode::TranslationNotConfigured,
            "DeepL API Key is invalid or does not have API access.",
        ),
        ...
        _ => (
            ErrorCode::TranslationFailed,
            "DeepL rejected the connection test.",
        ),
    };
...
fn hex_decode(text: &str) -> AppResult<Vec<u8>> {
    ...
    if text.len() % 2 != 0 {
        return Err(AppError::new(
            ErrorCode::DecryptFailed,
            "DeepL encrypted config is malformed.",
        ));
    }
...
fn protect_secret(secret: &str) -> AppResult<String> {
    ...
    if ok == 0 || output.pbData.is_null() {
        return Err(AppError::new(
            ...
            "Windows could not encrypt the DeepL API Key.",
        ));
    }
...
fn unprotect_secret(protected_hex: &str) -> AppResult<String> {
    ...
    if ok == 0 || output.pbData.is_null() {
        return Err(AppError::new(
            ErrorCode::DecryptFailed,
            "Windows could not decrypt the DeepL API Key.",
        ));
    }
    ...
    String::from_utf8(bytes).map_err(|_| {
        AppError::new(
            ErrorCode::DecryptFailed,
            "DeepL API Key could not be decoded.",
        )
    })
}
